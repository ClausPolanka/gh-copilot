...
class EndToEndTests {
    ...
    fun `a user enters text containing latin alphabetic words while ignoring stop words`() {
        ...
        main()
        assertContains(uiOutput(), "The text contains 7 word(s), 6 of them unique.")
    }

    @Test
    fun `a user provides a file as a user input source`() {
        main(arrayOf("my_text.txt"))
        assertContains(uiOutput(), "The text contains 4 word(s), 4 of them unique.")
    }
    ...
    fun `a user enters a word containing numbers`() {
        aUserEnters("Ma3ry")
        main()
        assertContains(uiOutput(), "The text contains 0 word(s), 0 of them unique.")
    }
    ...
    fun `a user enters a word ending with a punctuation mark`() {
        aUserEnters("Mary?")
        main()
        assertContains(uiOutput(), "The text contains 1 word(s), 1 of them unique.")
    }
